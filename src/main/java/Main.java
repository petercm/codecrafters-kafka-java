        ...
        try {
            ...
            while (!clientSocket.isClosed()) {
                ...
                */
                ...
                ByteBuffer mis = ByteBuffer.wrap(dis.readNBytes(messageSize));

                short apiKey = mis.getShort();
                short requestApiVersion = mis.getShort();
                int correlationId = mis.getInt();
                short clientIdLength = mis.getShort();
                String clientId = null;
                if (clientIdLength >= 0) {
                    clientId = new String(mis.get(new byte[clientIdLength]).array());
                }
                //TODO process tag buffer? Test calls have remaining bytes after extracting clientId
